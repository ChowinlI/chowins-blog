let endpos = (x, y) => {
  ...
  let value = random(20, 150)
  let radius = [-1, 1][random(0, 1)] * value
...
export default class Circle{
  ...
  moveFun(start, end, current) {
    ...
    let m = random(20, 25)
    let f = random(15, 20)
    let ff = start.x + ~~((end.x - start.x) / 3)
    let mm = ff + ~~((end.x - start.x) / 3)
    let ratio = end.x >= start.x ? (Math.max(current, ff) === current ? (Math.max(current, mm) === current ? s : m) : f) : (Math.max(current, ff) === current ? f : (Math.max(current, mm) === current ? m : s))
    let mp = {
        x: end.x - start.x,
        y: end.y - start.y
    }
  ...
  move() {
    let movepos = this.moveFun(this.opos, this.tpos, this.x)
